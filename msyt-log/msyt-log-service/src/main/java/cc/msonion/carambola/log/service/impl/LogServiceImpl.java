...
import cc.msonion.carambola.log.service.LogService;
 ...
 * @Date: 2017年7月16日 下午1:54:36
 * @Version: V2.0.0
 * @Modify-by: HeroCao hero-cao@msyc.cc
 * @Modify-date: 2017年7月16日 下午1:54:36
...
//    @Value("${msOnion.isPrintToConsole}")
    private boolean isPrintToConsole = false; // MsOnionLogger 已经通过全局配置拦截日志，这里是服务，不需要处理！！！
    ...
    public void info(String className, Exception ex, String logMsg) {
        try {
            ...
            if (isPrintToConsole) {
                ...
                LOGGER.info(String.format("info#%s", msg));
            }
        } catch (Exception e) {
    ...
    public void warn(String className, Exception ex, String logMsg) {
        try {
            ...
            if (isPrintToConsole) {
                ...
                LOGGER.warn(String.format("warn#%s", msg));
            }
        } catch (Exception e) {
            // 当前已经是日志中心，因此只能打印异常堆栈了
            e.printStackTrace();
    ...
    public void debug(String className, Exception ex, String logMsg) {
        try {
            ...
            if (isPrintToConsole) {
                ...
                LOGGER.debug(String.format("debug#%s", msg));
            }
        } catch (Exception e) {
            // 当前已经是日志中心，因此只能打印异常堆栈了
            e.printStackTrace();
