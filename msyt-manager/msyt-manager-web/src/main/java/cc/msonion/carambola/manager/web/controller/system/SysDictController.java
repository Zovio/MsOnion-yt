 ...
 */
@Controller
public class SysDictController extends MsOnionBaseAppController {
    ...
    public String toList(HttpServletRequest req, HttpServletResponse res, Model model) {
        try {
            ...
            this.getMsOnionLogger().info(getClass().getName(), e, "### 查询数据字典失败。");
            return getError();
        }
     ...
     * @return 访问路径
     */
    @RequestMapping(value = "/sysDict/{type}/{idx}")
    public String toSaveOrEditOrViewMenu(HttpServletRequest req, HttpServletResponse res,
        ...
        try {
            ...
            if (ManagerConstants.ADD.equals(type)) {
                ...
                SysDataDictionary sysDataDictionary = this.sysDataDictionaryService.queryByPrimaryKey(msOnionApiVersion, Long.valueOf(idx));
                model.addAttribute("sysDataDictionary", sysDataDictionary);
                model.addAttribute("type", type);
            } else {
                return getError();
            }
        } catch (MsOnionException e) {
            this.getMsOnionLogger().info(getClass().getName(), e, "### 进入数据字典操作页失败。");
            return getError();
        }
        return "/system/dict/saveOrEditOrViewDict";
    }
     ...
     */
    @RequestMapping(value = "/sysDict/updateStatus/{idxStr}/{statusStr}", method = {RequestMethod.POST})
    @ResponseBody
    public MsOnionResult updateStatus(HttpServletRequest req, HttpServletResponse res,
        ...
        try {
            ...
            int result = sysDataDictionaryService.updateStatus(msOnionApiVersion, idx, status);
            if (result > 0) {
    ...
    public Map<String, Object> paging(HttpServletRequest req, HttpServletResponse res, String page, String rows) {
        ...
        try {
            ...
            if (StringUtils.isNotBlank(dictCode)) {
                criteria.andDictCodeLike("%" + dictCode + "%");
            }
            ...
            MsOnionResultAdapter msOnionResultAdapter = sysDataDictionaryService.queryListByPageForResult(msOnionApiVersion, example,
     ...
     * @param res   HttpServletResponse  对象
     * @param model Model实例对象
     * @param sysDataDictionary  sysDataDictionary 对象
     * @return MsOnionResult对象
     */
    @RequestMapping(value = "/sysDict/do-saveOrEdit", method = RequestMethod.POST)
    @ResponseBody
    public MsOnionResult doEdit(HttpServletRequest req, HttpServletResponse res, Model model, SysDataDictionary sysDataDictionary) {
        try {
            Member currentUser = CurrentUserUtils.getCurrentUser();
            sysDataDictionary.setCreateByMemberIdx(currentUser.getIdx());
            sysDataDictionary.setUpdateByMemberIdx(currentUser.getIdx());

            MsOnionApiVersion msOnionApiVersion = MsOnionApiVersionUtils.getApiVersion();
            msOnionApiVersion.setRequestApiVersion(ManagerConstants.API_VERSION);
            MsOnionResult msOnionResult = sysDataDictionaryService.saveOrUpdate(msOnionApiVersion, sysDataDictionary);
