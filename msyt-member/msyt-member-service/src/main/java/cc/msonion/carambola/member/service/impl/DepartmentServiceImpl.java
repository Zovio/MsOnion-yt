 ...
 */
@Service
public class DepartmentServiceImpl extends MsOnionServiceExt<Department, DepartmentExample> implements DepartmentService {


    private static final long serialVersionUID = 8076885422483910381L;
    ...
    public MsOnionResult inspectParameter(MsOnionApiVersion apiVersion,
        ...
        try {
            ...
            String msg = null;
            if (ParamTypeConstants.DEPT_TYPE_CODE == type) {
                criteria.andCodeEqualTo(param);
                msg = ParamTypeConstants.DEPT_TYPE_CODE_MSG;
            } else {
                return new MsOnionResult(MsOnionStatusConstants.STATUS_400,
                        MsOnionMessageConstants.MESSAGE_PARAMETER_TYPE_ILLEGAL, type);
            }
            // 查询
            List<Department> list = this.queryByExample(apiVersion, example);
    ...
    @Override
    public MsOnionResult saveOrUpdate(MsOnionApiVersion apiVersion, Department department) throws MsOnionIllegalArgumentException, MsOnionException {
        try {
            if (null == department) {
                return MsOnionResult.build(MsOnionStatusConstants.STATUS_400, MessageConstants.MESSAGE_INFO_NULL);
            }

            if (department.getIdx() == null) {
                // add
                department.setStatus(MsOnionTableRecordStatus.NORMAL.getValue());
                String valiateMsg = MsOnionPojoValidatorUtils.validatePojo(department);
                if (StringUtils.isNotEmpty(valiateMsg)) {
                    return MsOnionResult.build(MsOnionStatusConstants.STATUS_400, valiateMsg);
                }
                // 查询数据库，校验菜单编码
                MsOnionResult moResult = this.inspectParameter(apiVersion, department.getCode(), ParamTypeConstants.DEPT_TYPE_CODE);
                ...
                Department dept = this.queryByPrimaryKey(apiVersion, department.getIdx());
                // 这些属性排除不需要copy
                ...
                BeanUtils.copyProperties(department, dept, excludeArr);

                int result = this.updateByPrimaryKey(apiVersion, dept);
                if (result > 0) {
                    this.getMsOnionLogger().info(this.getClass().getName(), "## 角色更新成功, result = " + result);
                    return MsOnionResult.ok(dept);
                }
            }
