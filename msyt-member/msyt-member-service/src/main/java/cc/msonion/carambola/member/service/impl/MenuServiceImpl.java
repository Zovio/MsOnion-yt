 ...
 */
@Service
public class MenuServiceImpl extends MsOnionServiceExt<Menu, MenuExample> implements MenuService {
    ...
    public List<Menu> getChildMenusByPidx(MsOnionApiVersion apiVersion, Long pidx) throws MsOnionIllegalArgumentException, MsOnionException {
        try {
            ...
            example.setOrderByClause("zindex asc,update_time desc");
     ...
     */
    @Override
    public List<Long> getChildIdxsByPidx(MsOnionApiVersion apiVersion, Long pidx) throws MsOnionIllegalArgumentException, MsOnionException {
        List<Menu> menus = getChildMenusByPidx(apiVersion, pidx);
        List<Long> idxs = new ArrayList<>();
        idxs.add(pidx);
        if (MsOnionCollectionUtils.isNotEmpty(menus)) {
            while (MsOnionCollectionUtils.isNotEmpty(menus)) {
                List<Menu> tempChilds = new ArrayList<>();
                for (Menu menu : menus) {
                    idxs.add(menu.getIdx());
                    List<Menu> templist = getChildMenusByPidx(apiVersion, menu.getIdx());
                    if (MsOnionCollectionUtils.isNotEmpty(templist)) {
                        for (Menu obj2 : templist) {
                            tempChilds.add(obj2);
                        }
                    }
                }
    ...
    public MsOnionResult getMenuViewObject(MsOnionApiVersion apiVersion,
        ...
        try {
            ...
            topMenu.setIdx(ParamTypeConstants.TOP_MENU_IDX);
            MenuViewObject menuViewObject = genTreeMenu(apiVersion, idxList, topMenu);
     ...
     */
    @Override
    public MsOnionResult getMenusByLevel(MsOnionApiVersion apiVersion, Short level) throws MsOnionIllegalArgumentException, MsOnionException {
        try {
            ...
            example.setOrderByClause("zindex asc,update_time desc");
            List<Menu> menus = queryByExample(apiVersion, example);
            return MsOnionResult.ok(menus);
        } catch (MsOnionIllegalArgumentException e) {
            this.getMsOnionLogger().error(getClass().getName(), e, "## 通过菜单菜单级别查询菜单集合失败");
            return MsOnionResult.build(MsOnionStatusConstants.STATUS_400, "## 通过菜单菜单级别查询菜单集合失败");
        } catch (MsOnionException e) {
            this.getMsOnionLogger().error(getClass().getName(), e, "## 通过菜单菜单级别查询菜单集合失败");
            return MsOnionResult.build(MsOnionStatusConstants.STATUS_400, "## 通过菜单菜单级别查询菜单集合失败");
        }
     ...
     * @throws MsOnionException                异常
     */
    private MenuViewObject genTreeMenu(MsOnionApiVersion apiVersion,
                                       List<Long> idxList, Menu m) throws MsOnionIllegalArgumentException, MsOnionException {
        MenuViewObject mvo = new MenuViewObject();
        MenuToViewObjectUtil.menuToVo(m, mvo);

        List<Menu> childMenuList = getChildMenusByPidx(apiVersion, m.getIdx());
        if (MsOnionCollectionUtils.isNotEmpty(childMenuList)) {
            for (Menu menu : childMenuList) {
                if (MsOnionCollectionUtils.isEmpty(idxList) || idxList.contains(menu.getIdx())) {
                    MenuViewObject t = genTreeMenu(apiVersion, idxList, menu);
